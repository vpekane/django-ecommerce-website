...
class Basket(models.Model):
    ...
    def create_order(self, billing_address, shipping_address):
        ...
        logger.info(
            f"Creating order for basket_id={self.id}, shipping_address_id={shipping_address.id}"
            f", billing_address_id={billing_address.id}"
        )
        order_data = {
            "user": self.user,
        ...
        for line in self.basketline_set.all():
            for item in range(line.quantity):
                ...
                count += 1
        logger.info(
            f"Created order with id={order.id} and lines_count={count}"
        )
